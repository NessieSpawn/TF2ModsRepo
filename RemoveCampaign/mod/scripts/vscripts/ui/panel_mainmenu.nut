...
void function TrackInstallProgress()
{
	...
	// just for fun: show a random install progress
	/*
	...
	*/
	while ( true )
	{
		if ( !IsStryderAuthenticated() ) // if not connected, don't overlap hud text
			RuiSetFloat( rui, "installProgress", 1.0 )
		else
		{
			float randomFrac = RandomFloatRange( 0.0, 1.0 )
			RuiSetFloat( rui, "installProgress", randomFrac )
		}
		
		WaitFrame() // hopefully won't stuck
	}
}

bool function IsStryderAuthenticated()
{
	// always return false to remove other prompts
