...
void function ClGamemodeColiseum_ColiseumIntro_Internal( int enemyWinStreak, int enemyWins, int enemyLosses )
{
	...
	RuiSetString( rui, "playerName0", GetPlayerAnonymousName( player ) )
	...
	RuiSetString( rui, "playerName1", GetPlayerAnonymousName( player ) )
...
void function Epilogue_OnEnter()
{
	...
	AnnouncementData announcement = Announcement_Create( Localize( "#COLISEUM_WINNER", GetPlayerAnonymousName( winningPlayer ) ) )
	...
	RuiSetString( rui, "playerName", GetPlayerAnonymousName( winningPlayer ) )
