	...
	for ( ;; )
	{
		...
		if ( IsValid( rider ) && rider.IsPlayer() )
		{
			...
			if ( rider.GetTeam() == player.GetTeam() )
			{
				...
				RuiSetString( rui, "playerName", GetPlayerAnonymousName( rider ) )
...
function DidUpdateRodeoRideNameAndIcon( entity cockpit, entity player, var rui )
{
	...
	if ( titan.GetTeam() == player.GetTeam() )
	{
		...
		*/
		if ( titan.IsPlayer() )
			playerName = GetPlayerAnonymousName( titan )
		else
		{
			entity bossPlayer = titan.GetBossPlayer()
			if ( IsValid( bossPlayer ) )
				playerName = GetPlayerAnonymousName( bossPlayer )
		}
...
string function GetTitanName( entity titan )
{
	...
	*/
	if ( titan.IsPlayer() )
		return GetPlayerAnonymousName( titan )

	entity bossPlayer = titan.GetBossPlayer()
	if ( IsValid( bossPlayer ) )
		return GetPlayerAnonymousName( bossPlayer )
