...
// modified: make this supports multiple callbacks
//global function SetScoreboardUpdateCallback
global function AddScoreboardUpdateCallback
...
struct {
	...
	array<void functionref(entity,var)> scoreboardUpdateCallbacks
} file
	...
	for ( ;; )
	{
		...
		foreach ( team, players in teamPlayers )
		{
			...
			foreach ( entity player in players )
			{
				...
				foreach ( callbackFunc in file.scoreboardUpdateCallbacks )
					callbackFunc( player, rui )
...
*/
void function AddScoreboardUpdateCallback( void functionref( entity, var ) func )
{
	if ( !file.scoreboardUpdateCallbacks.contains( func ) )
		file.scoreboardUpdateCallbacks.append( func )
}