...
string function FD_CheckPlayerMoney( entity player )
{
	...
	return Localize( "#PLAYER_CASH_HOLD_USE", minint( money, 100 ), GetPlayerAnonymousName( player ) )
}
...
void function FD_PlayerHealedHarvester( entity ent, var info )
{
	...
	AnnouncementData announcement = Announcement_Create( Localize( "#FD_PLAYER_HEALED_HARVESTER", GetPlayerAnonymousName( ent ) ) )
	Announcement_SetStyle( announcement, ANNOUNCEMENT_STYLE_SWEEP )
	Announcement_SetTitleColor( announcement, TEAM_COLOR_FRIENDLY )
	Announcement_SetSoundAlias( announcement, SFX_HUD_ANNOUNCE_QUICK )
	// modified for anonymous mode
	//Announcement_SetOptionalTextArgsArray( announcement, [ ent.GetPlayerName() ] )
	Announcement_SetOptionalTextArgsArray( announcement, [ GetPlayerAnonymousName( ent ) ] )
	Announcement_SetPurge( announcement, true )
...
void function FD_PlayerBoostedHarvesterShield( entity ent, var info )
{
	...
	AnnouncementData announcement = Announcement_Create( Localize( "#FD_PLAYER_BOOSTED_SHIELD", GetPlayerAnonymousName( ent ) ) )
...
void function FD_TurretRepair( entity ent, var info )
{
	...
	if ( turretOwner == GetLocalViewPlayer() )
	{
		...
		AnnouncementData announcement = Announcement_Create( Localize( "#HUD_TURRET_REPAIRED", GetPlayerAnonymousName( ent ) ) )
		Announcement_SetStyle( announcement, ANNOUNCEMENT_STYLE_SWEEP )
		Announcement_SetTitleColor( announcement, TEAM_COLOR_FRIENDLY )
		Announcement_SetSoundAlias( announcement, SFX_HUD_ANNOUNCE_QUICK )
		Announcement_SetPurge( announcement, true )
		...
		Obituary_Print_Generic( "#HUD_TURRET_REPAIRED", GetPlayerAnonymousName( ent ), <255, 255, 255>, OBITUARY_COLOR_FRIENDLY )
	...
	else if ( ent != turretOwner )
	{
		...
		Obituary_Print_Generic( Localize( "#HUD_TURRET_REPAIRED_OTHER", GetPlayerAnonymousName( ent ), GetPlayerAnonymousName( turretOwner ) ), "", <255, 255, 255>, OBITUARY_COLOR_FRIENDLY )
	}
}
...
void function FD_BoughtItem( entity ent, var info )
{
	...
	Obituary_Print_Generic( Localize( "#HUD_BOUGHT_ITEM", GetPlayerAnonymousName( ent ), Localize( burnReward.localizedName ) ), "", <255, 255, 255>, OBITUARY_COLOR_FRIENDLY )
}
...
void function FD_GotMoney( entity ent, var info )
{
	...
	Obituary_Print_Generic( Localize( "#HUD_GOT_MONEY", string( info ), GetPlayerAnonymousName( ent ) ), "", <255, 255, 255>, OBITUARY_COLOR_FRIENDLY )
}
...
void function FD_PlayerReady( entity ent, var info )
{
	...
	Obituary_Print_Generic( "#FD_PLAYER_READY", GetPlayerAnonymousName( ent ), <255, 255, 255>, OBITUARY_COLOR_FRIENDLY )
}
...
void function ServerCallback_UpdateGameStats( int p, int a, float v, int suitIndex )
{
	...
	awardData.playerName = GetPlayerAnonymousName( player )
	awardData.awardID = a
...
void function ServerCallback_FD_NotifyMVP( int playerHandle )
{
	...
	if ( "FDWaveMVP" in file.waveAwardRuis )
	{
		...
		RuiSetString( rui, "itemText", Localize( "#SCORE_EVENT_WAVE_MVP_OTHER", GetPlayerAnonymousName( player ) ) )
	}
}
