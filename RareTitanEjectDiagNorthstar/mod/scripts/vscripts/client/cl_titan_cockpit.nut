...
struct
{
	...
	// won't be accurate if player only changed camo but not tactical ability, just for fun
	//int lastPilotCamo
...
void function UpdateLastPlayerSettings( entity player )
{
	if ( IsPilot( player ) )
	{
		...
		//file.lastPilotCamo = player.GetCamo()
	}
	//print( "file.lastPilotSettings: " + file.lastPilotSettings )
	//print( "file.lastPilotCamo: " + file.lastPilotCamo )
}
...
void function DisplayFrontierRank( bool isFirstBoot = true )
{
	...
	foreach ( index, item in titanUpgrades )
	{
		...
		#if HAS_TITAN_FRAMEWORK
			if (!IsBaseTitan(titanClass) || !IsSubItemLocked( GetLocalClientPlayer(), item.ref, item.parentRef ) )
		#else
...
entity function CreateCockpitBody( entity cockpit, entity player, entity cockpitParent )
{
	...
	// needs debug this
	//print( "bodySettings: " + bodySettings )
	...
	#endif
	//print( "bodyModelName: " + string( bodyModelName ) )
...
string function RollRandomEjectString()
{
	...
	const float CHANCE_FOR_RARE = 100.0
